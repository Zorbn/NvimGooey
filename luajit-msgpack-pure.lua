-- From: https://github.com/catwell/luajit-msgpack-pure, MIT license.
...
local type_for = function(n)
  ...
  else print("Unsupported type:", n) assert(false) end
end
...
unpackers.ext8 = function(buf,offset)
  if offset + 1 >= buf.size then return nil,nil end
  local n = unpack_number(buf,offset,"uint8_t *",1)
  if offset + 2 + n >= buf.size then return nil,nil end
  return offset+3+n,ffi.string(buf.data+offset+1,n+2)
end
...
unpackers.str8 = function(buf,offset)
  if offset + 1 >= buf.size then return nil,nil end
  local n = unpack_number(buf,offset,"uint8_t *",1)
